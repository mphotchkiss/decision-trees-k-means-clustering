...
for m in range(M):
  ...
  # PART A CODE
  X_data = np.empty(X_train.shape)
  y_data = np.empty(y_train.shape)
  for i in range(X_train.shape[0]):
    index = np.random.randint(X_train.shape[0]-1)
    X_data[i] = X_train[index]
    y_data[i] = y_train[index]

  #Fit the model and store it's predictions
  clf = tree.DecisionTreeClassifier(criterion="entropy") #PART B CODE: max_features=X_data.shape[1]-20
