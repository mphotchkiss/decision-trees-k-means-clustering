...
def initalizeCentroids(dataset, k):
  ...
  for i in range(k):
    centroids[i] = dataset[np.random.randint(dataset.shape[0])]
  return centroids
...
def computeAssignments(dataset, centroids):
  distances = np.empty((centroids.shape[0], dataset.shape[0]))
  index = 0
  for centroid in centroids:
    distances[index] = np.linalg.norm(dataset-centroid, ord=2, axis=1)
    index += 1
...
def updateCentroids(dataset, centroids, assignments):
  counts = np.empty(centroids.shape[0])
  for i in range(0, centroids.shape[0]):
    indeces = np.transpose(assignments==i).nonzero()
    assigned_points = np.take(dataset, indeces, axis=0)
    new_centroid = np.mean(assigned_points, axis=1)
    centroids[i] = new_centroid
    counts[i] = indeces[0].shape[0]
...
def calculateSSE(dataset, centroids, assignments):
  ...
  index = 0
  for data in dataset:
    sse += np.linalg.norm(data - centroids[assignments[index]], ord=2, axis=0)
    index += 1
  return sse
